...
export const api = {
  ...
  getPlaylistV2: (id: string, limit = 100) =>
    request<PlaylistInfo>(`/api/playlist-v2?id=${encodeURIComponent(id)}&limit=${limit}`),
  ...
  getWatchPlaylist: (videoId: string, playlistId?: string, limit = 25) => {
    let url = `/api/watch/${encodeURIComponent(videoId)}?limit=${limit}`;
    if (playlistId) url += `&playlistId=${encodeURIComponent(playlistId)}`;
    return request<{ tracks: QueueItem[]; playlistId: string }>(url);
  },
};
